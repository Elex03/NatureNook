...
import sys
from src.Engine.finished.Main.camera import Camera
...
class GraphicsEngine:
    def __init__(self, win_size=(840, 480)):
        ...
        self.BackgroundSound = pg.mixer.Sound('finished/Main/resources/sounds/Nature.mp3')
        self.RadioAurora = pg.mixer.Sound('finished/Main/resources/sounds/Dreams.mp3')
        # renderer
    ...
    def run(self):
        while True:
            ...
            self.check_events()
            self.camera.update()
            self.scene.update(self.position_camera)  # Cambiado app a self
            self.render()
